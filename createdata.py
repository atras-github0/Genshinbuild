...
async def create(uid,charaindex,scoretype):
    async with enka:
        ...
        datajson += '"Status": {"HP": ' + str(chara.stats.FIGHT_PROP_MAX_HP.to_rounded()) + ',"攻撃力" : ' + str((chara.stats.FIGHT_PROP_BASE_ATTACK.to_rounded()) * (1 + chara.stats.FIGHT_PROP_ATTACK_PERCENT.value) + chara.stats.FIGHT_PROP_ATTACK.to_rounded()) + ',"防御力" : ' + str((chara.stats.FIGHT_PROP_BASE_DEFENSE.to_rounded()) * (1 + chara.stats.FIGHT_PROP_DEFENSE_PERCENT.value) + chara.stats.FIGHT_PROP_DEFENSE.to_rounded()) + ',"元素熟知" : ' + str(chara.stats.FIGHT_PROP_ELEMENT_MASTERY.to_rounded()) + ',"会心率" : ' + str(chara.stats.FIGHT_PROP_CRITICAL.to_percentage()) + ',"会心ダメージ" : ' + str(chara.stats.FIGHT_PROP_CRITICAL_HURT.to_percentage()) + ',"元素チャージ効率" : ' + str(chara.stats.FIGHT_PROP_CHARGE_EFFICIENCY.to_percentage())+ ','
        elementDMG = 0
        elementDMG2 = ""
