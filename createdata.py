...
async def create(uid,charaindex,scoretype):
    async with enka:
        ...
        datajson += '"Status": {"HP": ' + str(chara.stats.FIGHT_PROP_MAX_HP.to_rounded()) + ',"攻撃力" : ' + str(round((chara.stats.FIGHT_PROP_BASE_ATTACK.value) * (1 + chara.stats.FIGHT_PROP_ATTACK_PERCENT.value) + chara.stats.FIGHT_PROP_ATTACK.value)) + ',"防御力" : ' + str(round((chara.stats.FIGHT_PROP_BASE_DEFENSE.value) * (1 + chara.stats.FIGHT_PROP_DEFENSE_PERCENT.value) + chara.stats.FIGHT_PROP_DEFENSE.value)) + ',"元素熟知" : ' + str(chara.stats.FIGHT_PROP_ELEMENT_MASTERY.to_rounded()) + ',"会心率" : ' + str(chara.stats.FIGHT_PROP_CRITICAL.to_percentage()) + ',"会心ダメージ" : ' + str(chara.stats.FIGHT_PROP_CRITICAL_HURT.to_percentage()) + ',"元素チャージ効率" : ' + str(chara.stats.FIGHT_PROP_CHARGE_EFFICIENCY.to_percentage())+ ','
        elementDMG = 0
        elementDMG2 = ""
